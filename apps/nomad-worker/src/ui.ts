...
function page(title: string, body: string, formActionOrigins: string[] = []): Response {
  return new Response(`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#145f57"><meta name="apple-mobile-web-app-capable" content="yes"><meta name="apple-mobile-web-app-title" content="Nomad"><link rel="apple-touch-icon" href="/icon.svg"><link rel="manifest" href="/manifest.webmanifest"><title>${esc(title)} · Nomad</title><style>${style}</style></head><body><main>${body}<footer class="muted"><a href="/privacy">Privacy</a> · <a href="/terms">Pilot terms</a></footer></main><script src="/register.js" defer></script></body></html>`, {
    headers: {
      ...
      "Content-Security-Policy": `default-src 'none'; base-uri 'none'; form-action 'self'${formActionOrigins.length ? ` ${formActionOrigins.join(" ")}` : ""}; frame-ancestors 'none'; img-src 'self'; style-src 'unsafe-inline'; script-src 'self'; manifest-src 'self'; connect-src 'self'`,
    },
  });
}
...
function fieldForm(csrf: string, field?: Field) {
  ...
  return `<form method="post" action="${editing ? "/fields/edit" : "/fields/new"}">
    ...
    <p class="muted">Save one note that ChatGPT, Claude, or Gemini can use when helping you.</p>
    <div class="field-group"><label for="key">What is this note about?</label><input id="key" name="key" required maxlength="80" aria-describedby="key-help" placeholder="For example: How I like answers" value="${esc(field?.key ?? "")}"><small id="key-help" class="field-help">Give the note a short name, such as “How I like answers” or “My time zone.”</small></div>
    <div class="field-group"><label for="value">What should your assistants remember?</label><textarea id="value" name="value" required maxlength="4000" aria-describedby="value-help" placeholder="For example: Use short and simple words">${esc(field?.value ?? "")}</textarea><small id="value-help" class="field-help">Type the information you want them to use. Example: “Use short and simple words.”</small></div>
    <details${editing ? " open" : ""}><summary>Folder and privacy (optional)</summary>
    <div class="field-group"><label for="project">Folder</label><input id="project" name="project" required maxlength="80" aria-describedby="project-help" value="${esc(field?.project ?? "personal")}"><small id="project-help" class="field-help">A folder keeps similar notes together. Leave this as “personal” if you are unsure.</small></div>
    <div class="field-group"><label for="sensitivity">Privacy</label><select id="sensitivity" name="sensitivity" aria-describedby="sensitivity-help">
    ...
    </select><small id="sensitivity-help" class="field-help">Normal can be shared. Sensitive asks before changing it. Sealed is never shared with an assistant.</small></div></details><button type="submit">${editing ? "Save changes" : "Save note"}</button></form>`;
}
...
async function home(request: Request, env: Env, session: Session): Promise<Response> {
  ...
  return page("Passport", `<header><div><h1>Nomad passport</h1><div class="muted">${esc(user?.email)}</div></div><nav><a href="/files">Files</a><a href="/threads">Threads</a><a href="/metrics">Metrics</a><a href="/audit">Audit</a><a href="/account">Account</a></nav></header>
    <section><h2>Add a note</h2>${fieldForm(session.csrf)}</section>
...
export async function handleApp(request: Request, env: Env): Promise<Response> {
  ...
  if (!verifyForm(request, session, form, true)) return new Response("Invalid form origin or CSRF token.", { status: 403 });
