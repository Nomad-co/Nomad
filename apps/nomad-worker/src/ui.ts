...
function fieldForm(csrf: string, field?: Field) {
  ...
  return `<form method="post" action="${editing ? "/fields/edit" : "/fields/new"}">
    ...
    <div class="field-group"><label for="project">Project</label><input id="project" name="project" required maxlength="80" aria-describedby="project-help" value="${esc(field?.project ?? "personal")}"><small id="project-help" class="field-help">Groups related context. Use an area such as personal, Nomad, or job_search.</small></div>
    <div class="field-group"><label for="key">Field name</label><input id="key" name="key" required maxlength="80" aria-describedby="key-help" value="${esc(field?.key ?? "")}"><small id="key-help" class="field-help">Names what this fact means. Create a short label such as writing_style, timezone, or current_goal.</small></div>
    <div class="field-group"><label for="value">Value</label><textarea id="value" name="value" required maxlength="4000" aria-describedby="value-help">${esc(field?.value ?? "")}</textarea><small id="value-help" class="field-help">Enter the information assistants should use. Get it from your own preference, project notes, or a trusted document.</small></div>
    ...
    </select><small id="sensitivity-help" class="field-help">Normal fields can be shared. Sensitive fields require review for changes. Sealed fields are never returned to assistants.</small></div><button type="submit">${editing ? "Save changes" : "Add field"}</button></form>`;
}
