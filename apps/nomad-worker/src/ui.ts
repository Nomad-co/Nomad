...
function fieldForm(csrf: string, field?: Field) {
  ...
  return `<form method="post" action="${editing ? "/fields/edit" : "/fields/new"}">
    ...
    <label for="project">Project</label><input id="project" name="project" required maxlength="80" value="${esc(field?.project ?? "personal")}">
    <label for="key">Field name</label><input id="key" name="key" required maxlength="80" value="${esc(field?.key ?? "")}">
    <label for="value">Value</label><textarea id="value" name="value" required maxlength="4000">${esc(field?.value ?? "")}</textarea>
    <label for="sensitivity">Visibility</label><select id="sensitivity" name="sensitivity">
    ...
    </select><button type="submit">${editing ? "Save changes" : "Add field"}</button></form>`;
}
...
async function home(request: Request, env: Env, session: Session): Promise<Response> {
  ...
  return page("Passport", `<header><div><h1>Nomad passport</h1><div class="muted">${esc(user?.email)}</div></div><nav><a href="/files">Files</a><a href="/threads">Threads</a><a href="/metrics">Metrics</a><a href="/audit">Audit</a><a href="/account">Account</a></nav></header>
    <section><h2>Add a field</h2>${fieldForm(session.csrf)}</section>
...
async function consent(request: Request, env: Env, session: Session): Promise<Response> {
  ...
  return page("Connect assistant", `<header><h1>Connect an assistant</h1></header><section><p>This client requests ${pending.request.scope.includes("mcp:read") ? "read access to your unsealed fields" : "no read access"}${pending.request.scope.includes("mcp:write") ? " and permission to save new facts or propose changes" : ""}: <strong>${esc(client.clientName || "Unnamed client")}</strong>.</p>
    <p>You choose the label shown in the audit log. Enter “ChatGPT” or “Claude” only after checking which assistant opened this page.</p>
...
export async function handleApp(request: Request, env: Env): Promise<Response> {
  ...
  if (url.pathname === "/welcome" && request.method === "GET") return page("Welcome", `<header><h1>Your context, between assistants</h1><a href="/login">Sign in</a></header><section><p>Nomad is a passport for facts, files, and conversation handoffs you choose to share with ChatGPT and Claude.</p><p><a href="/setup">Set up a connector</a> · <a href="/limits">Read the limits</a> · <a href="/install">Install Nomad</a></p></section>`);
  ...
  if (url.pathname === "/setup" && request.method === "GET") return page("Setup", `<header><h1>Connect ChatGPT and Claude</h1><a href="/welcome">About Nomad</a></header><section><h2>ChatGPT</h2><ol><li>Open developer mode on ChatGPT web.</li><li>Add <code>${esc(env.PUBLIC_ORIGIN)}/mcp</code> as a connector.</li><li>Sign in and label the client ChatGPT.</li></ol><h2>Claude</h2><ol><li>Add a custom connector with the same MCP URL.</li><li>Sign in and label the client Claude.</li></ol><p>Ask directly: “Use Nomad Passport to load my saved context.”</p></section>`);
  if (url.pathname === "/limits" && request.method === "GET") return page("Limits", `<header><h1>Honest limits</h1><a href="/welcome">About Nomad</a></header><section><ul><li>The model decides when to call Nomad. Nomad cannot force it.</li><li>Nomad cannot silently watch or mirror a conversation.</li><li>Nomad does not synchronize ChatGPT or Claude's own memory.</li><li>Context already sent to a provider cannot be retracted.</li><li>ChatGPT connectors require a supported paid web plan.</li></ul></section>`);
  ...
  if (!verifyForm(request, session, form)) return new Response("Invalid form origin or CSRF token.", { status: 403 });
